...
class TestCommentApi(BaseTestTask):
    ...
    def test_create_comment_for_missing_task(self) -> None:
        ...
        response = self.make_authenticated_comment_request(
            "POST", account.id, "507f1f77bcf86cd799439011", token, data=comment_data
        )
    ...
    def test_create_comment_invalid_token(self) -> None:
        ...
        response = self.make_authenticated_comment_request(
            "POST", account.id, task.id, invalid_token, data=comment_data
        )
    ...
    def test_get_all_comments_with_comments(self) -> None:
        ...
        assert response.json["items"][0]["content"] == comments[2].content
        assert response.json["items"][1]["content"] == comments[1].content
        assert response.json["items"][2]["content"] == comments[0].content
    ...
    def test_delete_comment_success(self) -> None:
        account, token = self.create_account_and_get_token()
        task = self.create_test_task(account_id=account.id)
        created_comment = self.create_test_comment(account_id=account.id, task_id=task.id, content="To Delete")
    ...
    def test_comments_are_account_isolated_via_api(self) -> None:
        ...
        create_response = self.make_authenticated_comment_request(
            "POST", account1.id, task.id, token1, data={"content": "Account 1 Comment"}
        )
        account1_comment_id = create_response.json.get("id")

        get_response = self.make_cross_account_comment_request(
            "GET", account1.id, task.id, token2, comment_id=account1_comment_id
        )
        patch_response = self.make_cross_account_comment_request(
            "PATCH", account1.id, task.id, token2, comment_id=account1_comment_id, data={"content": "Hacked"}
        )
        delete_response = self.make_cross_account_comment_request(
            "DELETE", account1.id, task.id, token2, comment_id=account1_comment_id
        )
        ...
        verify_response = self.make_authenticated_comment_request(
            "GET", account1.id, task.id, token1, comment_id=account1_comment_id
        )
